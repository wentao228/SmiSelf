...
def tokenize_smiles(smiles: str) -> Iterator[SMILESToken]:
    ...
    while i < len(smiles):
        ...
        if i == len(smiles):
            # Rule: delete trailing hanging bond token
            break
            ...
            if r_idx == -1:
                # Rule: delete unclosed bracket token
                i += 1
                continue
            ...
            if bond_idx is not None:
                # Rule: delete hanging bond before branch token
                bond_idx = None
            token = SMILESToken(
...
def _derive_mol_from_tokens(mol, smiles, tokens, i):
    ...
    while tokens:
        ...
        if symbol_type == SMILESTokenTypes.DOT:
            ...
            if curr is None:
                # Rule: ignore invalid atom symbol token.
                i += 1
                continue
            ...
            # Rule: ignore non-atom token at chain start.
            i += 1
            continue
    ...
    if len(mol) == 0:
        # Rule: ignore empty SMILES fragment and continue.
        return i

    if branch_stack:
        # Rule: treat hanging '(' bracket as auto-closed at the end.
        while branch_stack:
            branch_stack.pop()
            prev_stack.pop()

    if ring_log:
        # Rule: remove hanging ring number.
        # Drop unresolved placeholders to avoid None edges in traversal.
        unresolved = {}
        for _, (_, latom, lpos) in ring_log.items():
            unresolved.setdefault(latom.index, []).append(lpos)
        for atom_idx, positions in unresolved.items():
            out_edges = mol.get_out_dirbonds(atom_idx)
            for pos in sorted(positions, reverse=True):
                if (0 <= pos < len(out_edges)) and (out_edges[pos] is None):
                    out_edges.pop(pos)
    return i
...
def _make_ring_bonds(mol, smiles, ltoken, latom, lpos, rtoken, ratom):
    if mol.has_bond(latom.index, ratom.index):
        # Ignore redundant ring closure on already-bonded atoms.
        # A placeholder was created when ltoken was seen, so remove it.
        out_edges = mol.get_out_dirbonds(latom.index)
        if (0 <= lpos < len(out_edges)) and (out_edges[lpos] is None):
            out_edges.pop(lpos)
        return
    ...
    if ((bonds[0] == bonds[1])
        ...
        # Rule: conservative fallback for mismatched ring bonds.
        # Remove the left placeholder and ignore this ring closure.
        out_edges = mol.get_out_dirbonds(latom.index)
        if (0 <= lpos < len(out_edges)) and (out_edges[lpos] is None):
            out_edges.pop(lpos)
        return
