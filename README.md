# SmiSelf
This repository contains the official code for our EMNLP 2025 Main paper:  
**How to Make Large Language Models Generate 100% Valid Molecules?**

---

## 🚧 Code Release Notice

Due to moving and attending the conference of Wen (First Author) over the past few months, the code is still being reorganized.  
The code will be released **within November 2025**.  

Thank you for your interest and recognition! 🙏  
Please stay tuned for updates and ⭐ star the repo if you would like to follow our progress.

---

## 📫 Contact

If you have any questions or collaboration interests, feel free to reach out or open an issue.

---

## 📖 Citation

```bibtex
@inproceedings{
tao2025how,
title={How to Make Large Language Models Generate 100\% Valid Molecules?},
author={Wen Tao and Jing Tang and Alvin Chan and Bryan Hooi and Baolong Bi and Nanyun Peng and Yuansheng Liu and Yiwei Wang},
booktitle={The 2025 Conference on Empirical Methods in Natural Language Processing},
year={2025},
url={https://openreview.net/forum?id=X750nHvTdi}
}
